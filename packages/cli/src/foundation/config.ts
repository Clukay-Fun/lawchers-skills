/**
 * 描述: Lawchers 全局配置解析与 home 路径解析
 * 主要功能:
 *     - 解析跨平台的 LAWCHERS_HOME 默认数据目录
 *     - 加载并合并默认、用户、项目和显式覆盖配置
 */
...
export async function loadConfig(options: LoadConfigOptions = {}): Promise<LoadedConfig> {
  ...
  const explicitConfig = paths.configFile ? await readRequiredConfig(paths.configFile) : undefined;

  return {
    config: mergeConfig(defaultConfig, userConfig, projectConfig, explicitConfig, options.overrides),
    home,
    paths
  };
}
...
async function readRequiredConfig(filePath: string): Promise<PartialLawchersConfig> {
  const config = await readOptionalConfig(filePath);
  if (!config) {
    throw new LawchersError("IO_ERROR", `Config file does not exist: ${filePath}`, {
      path: filePath
    });
  }
