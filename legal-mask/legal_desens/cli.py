...
def _apply_decisions(args: argparse.Namespace, fmt: str, decisions_file: str) -> int:
    ...
    if not isinstance(source_map, dict):
        print("Error: source map must be a JSON object", file=sys.stderr)
        return 1
    try:
        current_sha = hashlib.sha256(Path(args.input).read_bytes()).hexdigest()
    except OSError as e:
        print(f"Error reading source: {e}", file=sys.stderr)
        return 1
    if source_map.get("source_sha256") != current_sha:
        print("Error: source_sha256 mismatch or missing; rerun prepare and review the current source.", file=sys.stderr)
        return 1
