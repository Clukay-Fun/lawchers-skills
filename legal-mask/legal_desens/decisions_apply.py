...
def _load_decisions(path: str) -> List[dict]:
    ...
    for index, decision in enumerate(decisions):
        ...
        if decision.get("action") not in ("redact", "keep"):
            # Never silently ignore an unknown action (e.g. 'replace').
            # Index-only diagnostics avoid echoing user-supplied sensitive data.
            raise ValueError(f"Decision at index {index}: action must be redact or keep")
    return decisions
